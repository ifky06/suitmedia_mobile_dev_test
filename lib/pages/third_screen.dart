      ...
      body: BlocBuilder<UserListBloc, UserListState>(
        builder: (context, state) {
          if (state is UserListInitial) {
            BlocProvider.of<UserListBloc>(context).add(GetUserList(page, pageSize));
            return const Center(child: CircularProgressIndicator());
          } else if (state is UserListLoaded) {
            return UserListView(users: state.users, hasReachedMax: state.hasReachedMax);
          } else {
...
class UserListView extends StatefulWidget {
  ...
  const UserListView({super.key, required this.users, required this.hasReachedMax});
...
class _UserListViewState extends State<UserListView> {
  ...
  void initState() {
    ...
    _scrollController.addListener(() {
      ...
      if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
        _page++;
        BlocProvider.of<UserListBloc>(context).add(GetUserList(_page, widget.users.length));
      }
    });
  ...
  //   if (_scrollController.position.pixels == _scrollController.position.maxScrollExtent) {
  //     _page++;
  //     BlocProvider.of<UserListBloc>(context).add(FetchUserList(_page, widget.users.length));
  //   }
        ...
        itemBuilder: (context, index) {
          if (index < widget.users.length){
            return ListTile(
              ...
              onTap: (){
                BlocProvider.of<UserBloc>(context).add(GetUser(widget.users[index]));
